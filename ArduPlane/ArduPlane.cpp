...
/*        if(set_pith_max){
          ///   nav_pitch_cd = -4500;
                ///    nav_roll_cd = constrain_int32(nav_roll_cd, -roll_limit_cd, roll_limit_cd);
                 static uint32_t prt_cnt;
                 prt_cnt++;
                 // servo_out_val+=1;
                 // if(servo_out_val >=4000)
                 //    servo_out_val = 0;
                if(prt_cnt % 100 == 0)
                     gcs_send_text(MAV_SEVERITY_CRITICAL,"set_radio_out: 1987");
  ///               gcs_send_text_fmt(MAV_SEVERITY_CRITICAL,"set_radio_out: %d",servo_out_val);
              channel_pitch->set_pwm(1200);
